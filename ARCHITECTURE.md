...
8. Code shape must stay small and single-purpose.
   ...
   - A class may own only one responsibility. If it has more than one reason to
     ...
     behavior.
   - Prefer at most 5 parameters, methods around 30-70 lines, and classes around
     200-300 lines as review signals, not mechanical pass/fail limits.
   - Exceeding a signal requires a Locality and readability review. It does not
     by itself authorize a split, move, rename, or rewrite.
   - A shorter implementation is not automatically better. Do not compress code
     or introduce indirection merely to satisfy a number.
   - Existing large code is not automatic permission to refactor; use the
     structural approval process when ownership or behavior would move.
...
10. Tests are selected by risk, not prohibited by file type.

   - Add focused Java tests for pure ordering, fallback, protocol, allocation,
     and compatibility contracts when they can run without a Minecraft server.
   - Use integration runs or manual reproduction for mixin injection, rendering,
     networking lifecycle, and other runtime-only behavior.
   - Do not add production reset hooks or widen production visibility only for a
     test. Prefer pure helpers or package-local rule tests.
   - Every bug fix should have the smallest durable regression check that can
     prove the failed behavior without coupling to implementation details.
...
SparkTraits, and future faction-aware add-ons. SparkStrength and SparkAssist do
not currently compile against SparkFactionAPI; do not claim them as consumers
until code or metadata establishes that dependency.
...
- `impl/shop/`: custom faction shop access decisions and Wathe shop adapters.
...
## Public Policy Evaluation Contracts

Registration and evaluation order are public behavior. Preserve these rules
unless the owner explicitly approves a breaking API change and a migration plan:

| Seam | Evaluation contract | `null` / fallback contract |
| --- | --- | --- |
| Effective faction | Resolvers run in registration order; every non-null result becomes the input faction for later resolvers. | `null` keeps the current faction. Null player/component resolves to `sparkfactionapi:none`. |
| Economy, gun, target, blackout | Policies run in registration order; the first non-null answer wins. | `null` delegates to the next policy, then to capability/native fallback. |
| Instinct highlight | All policies are evaluated; highest numeric priority wins. Earlier registration wins equal-priority ties. | `null` contributes no candidate; no candidate falls through to normal highlight rules. |
| Custom faction win | Factions are checked by descending faction priority. `BLOCK` outranks every faction win; otherwise the first valid faction win is retained. | `null` and `NONE` are ignored; no retained result means `NONE`. |

The role-only `resolveEffectiveFaction(Role)` method is a compatibility alias
for base-faction lookup and is deprecated because a role has no player context.
New code must call `resolveBaseFaction(Role)` or the player-aware overload.

## External Compatibility Contract

| External project | Relationship | Current contract |
| --- | --- | --- |
| Wathe | Hard runtime and compile dependency | Compiled and tested against `wathe 1.5.6-spark-1.21.1`. Mixins bind to exact method/field shapes, so a broader loader range does not imply untested binary compatibility. |
| SparkWitch | Direct public API consumer | May import only `api/`; internal `impl/`, `component/`, `mixin/`, and `net/` packages are not downstream contracts. |
| SparkTraits | Optional faction-aware integration | Must use the public facade/API seam and tolerate the mod being absent. |
| SparkStrength | No current dependency | Do not add a FactionAPI dependency unless a concrete feature requires it. |
| SparkAssist | No current Java dependency | Identifier-level client compatibility does not make it a Java API consumer. |

When a Wathe update changes a referenced symbol, verify every mixin target and
accessor against that exact jar before changing the declared compatibility
range.
