...
## 2026-07-09 Contract Verification Repair

Status: Complete.

- Restored focused JUnit coverage for allocation, custom-win precedence,
  version protocol payloads, and public facade compatibility.
- Marked `resolveEffectiveFaction(Role)` as a backward-compatible deprecated
  alias and documented the player-aware/base-faction distinction.
- Recorded the exact policy ordering and fallback contracts, current downstream
  consumer matrix, and Wathe mixin-version risk in the live constitution.
- Added the existing `impl/shop/` owner to the target map.
- Replaced numeric code-shape gates and the blanket test ban with advisory
  Locality review and risk-based verification.
- Added `verifyArchitecture` to keep governance files, the public compatibility
  seam, and representative contract tests present.

No packet id, payload order, component id, NBT key, gameplay value, policy
ordering, registration order, fallback behavior, or unrelated role/faction
behavior changed.
